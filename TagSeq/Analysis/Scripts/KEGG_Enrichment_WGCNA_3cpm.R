...
d7_WGCNA_all   <- read.csv("Analysis/Output/WGCNA/3cpm/Day7/d7.WGCNA_ModulMembership.csv")
...
# Day 7 for loop ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::;;; #
Day7_WGCNA_sigmodules <- as.data.frame(c('brown','blue'))
for (i in 1:nrow(Day7_WGCNA_sigmodules)) {
  ...
  Pgenerosa_gene_calls <- length(unique(EnrichedGenes_2$Gene_terms)) # total genes called - use this to learn the percent merged with the C.gigas KEGG dataset (unique genes)
    ...
    # merge and calculate the percent mapping
  # Lets merge with the Cgigas genome! 
  EnrichedGenes_2$Gene_terms <- gsub("[[]", "", EnrichedGenes_2$Gene_terms)
  vector                 <- as.vector(EnrichedGenes_2$Gene_terms) # call the gene term as a vector 
  vector_with_asserts    <- paste("^", vector, "$", sep='') # assets help to call the exact beginning and end of the gene term in grep1 (exact mtatch!) - otheriwse grep will use an 'in it' like funciton and call any genes that contain the whole term in addition to extra string chracters
  x <- length(vector_with_asserts)
  x.2 <- x /2 
  Firsthalf <- vector_with_asserts[1:x.2]
  Sechalf   <- vector_with_asserts[x.2:x]
  EnrichedGenes_Cgigas_1 <- subset(Crass_gigas_genome_dataframe, grepl(paste("^",Firsthalf,"$", sep="", collapse= "|"), Gene_terms, ignore.case = TRUE)) # subset the C gigas genome by the common and exact match to gene terms (with sig GO enrichmetn from 'goseq'!) in the module of interest 
  EnrichedGenes_Cgigas_2 <- subset(Crass_gigas_genome_dataframe, grepl(paste("^",Sechalf,"$", sep="", collapse= "|"), Gene_terms, ignore.case = TRUE)) # subset the C gigas genome by the common and exact match to gene terms (with sig GO enrichmetn from 'goseq'!) in the module of interest 
  EnrichedGenes_Cgigas <- rbind(EnrichedGenes_Cgigas_1, EnrichedGenes_Cgigas_2)
  
  length(unique(EnrichedGenes_Cgigas$Gene_terms)) # 869 calls
  ...
  if (  nrow(as.data.frame(head(KEGG_cgigas))) > 0 ) {
    ...
    KEGGoutput <- as.data.frame(do.call(cbind.data.frame, df))
    KEGGoutput$GeneRatio <- gsub("/"," of ", KEGGoutput$GeneRatio)
    write.csv(KEGGoutput, file = paste("Analysis/Output/KEGG/WGCNA/3cpm/Day7_",modColor,"_KEGG_allgenes.csv", sep ='')) 
    ...
    df_3$Cgigas_KEGG_IDs <- paste("crg:", df_3$gene_IDs, sep='')
    df_final <- merge(df_3, Crass_gigas_genome_dataframe, by='Cgigas_KEGG_IDs')
    write.csv(df_final, file = paste("Analysis/Output/KEGG/WGCNA/3cpm/Day7_",modColor,"_KEGG_allgenes_unlisted.csv", sep ='')) 
