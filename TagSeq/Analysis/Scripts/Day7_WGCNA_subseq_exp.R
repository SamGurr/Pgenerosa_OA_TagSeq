---
  # title: "Day7_WGCNA" - for subseqent exposures WGGCNA results (without ambient)
  # author: "Samuel Gurr"
  # date: "January 8, 2021"
---
  
# NOTE: what is the purpose of this script and how does it differ from the full datase  ('_all' version)?  
...
d7.Treatment_Phenotype          <- Master.Treatment_Phenotype.data %>%  dplyr::filter(Date %in% 20190731) # split for day 7 data 
d7.Treatment_Phenotype_TARGETS  <- d7.Treatment_Phenotype %>% dplyr::filter(!Second_Treament == 'A')
d7.Treatment_Phenotype_TARGETS$Sample.Name # these are the sample names we want !! (ommitted ambient second encounter)

# call the column names with the rows in d7.Treatment_Phenotype.data_TARGETS$Sample.Name (the 'condenced' attribute of this script)
day7.counts.matrix_TARGETS <- day7.counts.matrix %>% dplyr::select(c('X', (one_of(d7.Treatment_Phenotype_TARGETS$Sample.Name))))
...
dim(day7.counts.matrix_TARGETS) # 8548  rows (genes) -   25  samples  not counting 'x' and 'Gene.ID'
(day7.counts.matrix_TARGETS)[1] # ommit the first line and transpose in the next line 
d7.data = as.data.frame(t(day7.counts.matrix_TARGETS[, -(1)])) # ommit all columns but samples and transpose
dim(d7.data) # 24 8548
...
dim(d7.Treatment_Phenotype_TARGETS) #  24 rows and 14 columns
names(d7.Treatment_Phenotype_TARGETS) # look at the 14 columns 
d7.Treatment_Phenotype.data = d7.Treatment_Phenotype_TARGETS[, -c(1:3,5,8)]; # remove columns that hold information we do not need. (i.e. tankks, Third treatment, All treatment group, etc.)
dim(d7.Treatment_Phenotype.data) # 24 rows and  9 columns
...
names(d7.data) = day7.counts.matrix_TARGETS$X # assigns column names (previous jsut numbered) as the gene ID 
rownames(d7.data) = names(day7.counts.matrix_TARGETS)[-(1)]; # assigns the row names as the sample ID
d7.data_matrix <- data.frame(day7.counts.matrix_TARGETS[,-1], row.names=day7.counts.matrix_TARGETS[,1]) 
...
dim(dds.d7_vst) #  8548 genes; 24  samples
...
png("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/Day7_ClusterTree_Precut.png", 1000, 1000, pointsize=20)
...
abline(h = 85, col = "red") # add line to plot to show the cut-off od outlier samples (40000) SG105 and SG55
dev.off()

# cut ethe tree and ommit from the reads data 
clust = cutreeStatic(sampleTree, cutHeight = 85, minSize = 10) # Determine cluster under the line
...
nSamples = nrow(dds.d7_vst) # number of samples == 19  - the cut tree removed 5 samples 
...
png("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/Day7_ClusterTree_Postcut.png", 1000, 1000, pointsize=20)
...
dim(dds.d7_vst) #   19 8548- transformed count data now  has 30 samples - 6 cut in the tree step above 
dim(d7.Treatment_Phenotype.data) #   24 9 - trait data has  24 samples - not yet cut! 
...
dim(d7.Traits) #  19 Samples 8 columns (primary and second treatment) - note: no Third treatment because that occured Days 14-21, after this sampling day
...
# d7.Traits.Secondary$A <- (d7.Traits.Secondary$Second_Treament == "A") # ommited earlier!
...
d7.Traits.Secondary <- d7.Traits.Secondary[,c(2:3)] # final dataset of 0,1 for teatment groups - Secondary only!
...
d7.Traits.Group <- d7.Traits.Group[,c(2:5)] # final dataset of 0,1 for teatment groups - Secondary groups only!
...
# All Traits
png("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/Day7_ClusterTree_AllTraits.png", 1000, 1000, pointsize=20)
...
png("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/Day7_ClusterTree_PrimaryTreatment.png", 1000, 1000, pointsize=20)
...
# Second treatment 
png("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/Day7_ClusterTree_SecondTreatment.png", 1000, 1000, pointsize=20)
...
png("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/Day7_ClusterTree_GroupTreatment.png", 1000, 1000, pointsize=20)
...
# Phys Only
png("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/Day7_ClusterTree_PhysOnly.png", 1000, 1000, pointsize=20)
...
# save data
save(dds.d7_vst, d7.Traits, file = "Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/d.7-dataInput.RData")
save(dds.d7_vst, d7.Traits.phys, file = "Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/d.7-dataInput_treatmentonly.RData")
save(dds.d7_vst, d7.Traits.treat, file = "Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/d.7-dataInput_physonly.RData")


# write the vst transformed data 
write.csv(dds.d7_vst, "Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/Day7_vstTransformed_WGCNAdata.csv") # write
...
# png to output 
png("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/Day7_ScaleTopology_SoftThresh.png", 1000, 1000, pointsize=20)
par(mfrow = c(1,2));
cex1 = 0.9;
...
dynamicColors = labels2colors(dynamicMods_sign) # add colors to module labels (previously numbers)
table(dynamicColors) # lets look at this table...
# Plot the dendrogram and colors underneath
sizeGrWindow(8,6)
plotDendroAndColors(geneTree_sign, dynamicColors, "Dynamic Tree Cut - SIGNED",
...
# Plot the result
sizeGrWindow(7, 6)
png("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/Day7_ClusterEigengenes.png", 1000, 1000, pointsize=20)
...
#
#=====================================================================================
MEDissThres = 0.67 # outide of the height threshold - will not affect any modules at this point
...
png("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/Day7_ClusterDendrogram_signed.png", 1000, 1000, pointsize=20)
plotDendroAndColors(geneTree_sign, cbind(dynamicColors, mergedColors),
...
save(MEs, moduleLabels, moduleColors, geneTree, file = "Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/Day7-networkConstruction-stepByStep.RData")
# write csv - save the module eigengenes
write.csv(MEs, file = "Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/d7.WGCNA_ModulEigengenes.csv")
table(mergedColors)
...
#                        TOMType = "signed",  
#                        saveTOMs = TRUE,
#                        saveTOMFileBase = "Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/d7.Geoduck", 
...
png("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/heatmaps/Day7_Treatments_Primary_heatmap2.png", 500, 1000, pointsize=20)
...
png("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/Day7_Treatments_Primary_heatmap.png", 500, 1000, pointsize=20)
pdf("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/heatmaps/Day7_Treatments_Primary_heatmap.pdf", width=5, height=6)
...
par(mar = c(8, 9.5, 5, 3));
# Display the correlation values within a heatmap plot
png("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/heatmaps/Day7_Treatments_Second_heatmap2.png", 1000, 500, pointsize=20)
...
png("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/heatmaps/Day7_Treatments_Second_heatmap.png", 500, 1000, pointsize=20)
pdf("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/heatmaps/Day7_Treatments_Second_heatmap.pdf", width=5, height=6)
...
par(mar = c(8, 9.5, 5, 3));
# Display the correlation values within a heatmap plot
png("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/heatmaps/Day7_Group_heatmap2.png", 1000, 500, pointsize=20)
...
png("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/heatmaps/Day7_Group_heatmap.png", 500, 1000, pointsize=20)
pdf("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/heatmaps/Day7_Group_heatmap.pdf", width=5, height=6)
...
#plot it
png("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/Day7_ME_Boxplot.png", 600, 1000, pointsize=20)
...
# AA treatment group
# AA = as.data.frame(d7.Traits.Group$AA); # Define variable containing the desired column 
# names(AA) = "AA"
...
# AM treatment group
AM = as.data.frame(d7.Traits.Group$AM); # Define variable containing the desired column 
names(AM) = "AM"
...
# AS treatment group
AS = as.data.frame(d7.Traits.Group$AS); # Define variable containing the desired column 
names(AS) = "AS"
...
# MA treatment group
# MA = as.data.frame(d7.Traits.Group$MA); # Define variable containing the desired column 
# names(MA) = "MA"
# rownames(dds.d7_vst)
...
# MM treatment group
MM = as.data.frame(d7.Traits.Group$MM); # Define variable containing the desired column 
names(MM) = "MM"
...
# MS treatment group
MS = as.data.frame(d7.Traits.Group$MS); # Define variable containing the desired column 
names(MS) = "MS"
...
length(colnames(dds.d7_vst)[moduleColors=="blue"]) # 188 total genes in the brown module
yellow_mod <- geneInfo_GROUPS %>%  dplyr::filter(moduleColor %in% 'blue')
# red <- red_mod[,c(3,4:7)]
...
yellow_mod_05 <- yellow_mod %>%  dplyr::filter(p.MM.yellow < 0.05) 
yellow_mod_05_MM <-yellow_mod_05[,c(8:13)]
nrow(yellow_mod_05_MM) # 78 total genes in the yellow module at 0.0001 module membership 

yellow_mod_05_MM$Gene.ID <- rownames(yellow_mod_05_MM)
yellow_mod_05_MM_melt <- melt(yellow_mod_05_MM, id=c("Gene.ID"))
yellow_mod_05_MM_melt$Primary <- substr(yellow_mod_05_MM_melt$variable, 4,4)
yellow_mod_05_MM_melt$Second <- substr(yellow_mod_05_MM_melt$variable, 5,5)

ggplot(yellow_mod_05_MM_melt, aes(x=Second, y=value, group=Gene.ID, color = Primary)) +
  theme(panel.grid=element_blank()) +
  scale_color_manual(values=c("#56B4E9","#D55E00")) +
  geom_line(size=0.1, alpha=0.2) +
  theme_bw() +
  ggtitle("Day 7 WGCNA: yellow module") +
...
length(colnames(dds.d7_vst)[moduleColors=="brown"]) # 1495 total genes in the brown module
...
# blue module - lower in AS than other modules 
length(colnames(dds.d7_vst)[moduleColors=="blue"]) # 595 total genes in the blue module
blue_mod <- geneInfo_GROUPS %>%  dplyr::filter(moduleColor %in% 'blue')
nrow(blue_mod) # 595 total genes in the blue module
blue_mod_05 <- blue_mod #%>%  dplyr::filter(p.MM.blue < 0.05) 
blue_mod_05_MM <-blue_mod_05[,c(8:13)]
nrow(blue_mod_05_MM) # 164 total genes in the blue module at 0.0001 module membership 

blue_mod_05_MM$Gene.ID <- rownames(blue_mod_05_MM)
blue_mod_05_MM_melt <- melt(blue_mod_05_MM, id=c("Gene.ID"))
blue_mod_05_MM_melt$Primary <- substr(blue_mod_05_MM_melt$variable, 4,4)
blue_mod_05_MM_melt$Second <- substr(blue_mod_05_MM_melt$variable, 5,5)

ggplot(blue_mod_05_MM_melt, aes(x=Second, y=value, group=Gene.ID, color = Primary)) +
  theme(panel.grid=element_blank()) +
  scale_color_manual(values=c("#56B4E9","#D55E00", "#D55E00")) +
  geom_line(size=0.1, alpha=0.2) +
  theme_bw() +
  ggtitle("Day 7 WGCNA: blue module") +
  ylab("Pearson's correlation of vst Expression and Treatment Group (genes with Module Membership < 0.001)") +
  facet_wrap(~Primary)
...
write.csv(geneInfo_GROUPS, file = "Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/d7.WGCNA_ModulMembership.csv")
...
# Load data 
d7_ModEigen <- read.csv("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/d7.WGCNA_ModulEigengenes.csv")
d7_Annot_ModuleMembership <-  read.csv("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/d7.WGCNA_ModulMembership.csv")
d7_vst_data <- read.csv("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/Day7_vstTransformed_WGCNAdata.csv")
...
for(i in 1:ncol(modules)) {
  ...
  ggsave(paste0("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/EigengenePlots/Day7_EigengenePlot_", module_color, ".png"))
}
...
for(i in 1:nrow(modcolor)) {
  ...
  pdf(paste("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/ModuleExpression_Treatment/day7_Exp_Module",modcolor[i,],".pdf"), width=6, height=6)
...
for(i in 1:nrow(modcolor)) {
  ...
  GO_module <- d7_Annot_ModuleMembership %>% dplyr::filter(moduleColor %in% modcolor[i,]) # %>%  dplyr::select("geneSymbol")
  ...
  MF_tile_plot <- mod_enriched.GO.05 %>% 
                    ...
                    mutate(term = fct_reorder(term, ontology)) %>%
                        ...
                        theme_classic2() + 
                        ggtitle(paste("GO: Day 7 WGCNA ME", modcolor[i,], sep ='')) +
                        theme(legend.position="none") 
  ...
  pdf(paste("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/goseq_modules/Day7_goseq_tiles_ME",modcolor[i,],".pdf", sep =''), width=25, height=50)
  ...
  # plot the output as geom_segment plots 
  library(tidyr)
  ...
  pdf(paste("Analysis/Output/WGCNA/10cpm/subseq_hypercapnia/Day7/goseq_modules/Day7_goseq_ME",modcolor[i,],".pdf", sep =''), width=8, height=5)
  print(Plot)
  dev.off()
}
