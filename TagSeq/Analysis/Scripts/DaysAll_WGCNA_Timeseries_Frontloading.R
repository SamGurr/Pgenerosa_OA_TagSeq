...
Master.Exp.Metadata                <- read.csv(file="Analysis/Data/Experiment_Metadata/Master_Phyenotype.and.Exp.Treatment_Metadata.csv", sep=',', header=TRUE) %>% 
                                            dplyr::select(c('Date', 'Sample.Name', 'Primary_Treatment', 'Second_Treament', 'Third_Treatment')) %>% 
...
Master.Exp.Metadata$Day_PrimTreatment  <- paste(Master.Exp.Metadata$Date, Master.Exp.Metadata$Primary_Treatment, sep = "_")
Master.Exp.Metadata$Day_PrimSecTreatment  <- paste(Master.Exp.Metadata$Date, Master.Exp.Metadata$Primary_Treatment, Master.Exp.Metadata$Second_Treament, sep = "_")
Master.Exp.Metadata$Day_FullTreatment  <- paste(Master.Exp.Metadata$Date, Master.Exp.Metadata$Primary_Treatment, Master.Exp.Metadata$Second_Treament,  Master.Exp.Metadata$Third_Treatment,sep = "_")
...
# (1) Full data all samples 
# ncol(Master.data_matrix) = 133
...
# ncol(Data_matrix_ModExp) # 91

# metadata
Exp.Metadata_ModExp   <- Master.Exp.Metadata_2 %>%  
  dplyr::filter(!Second_Treament %in% 'S') %>% 
  dplyr::mutate(PrimSecThird = paste( (gsub("_.*","", Day_Treatment)), "_", Primary_Treatment, Second_Treament, Third_Treatment, sep='')) 
Exp.Metadata_ModExp$PrimSecThird <-  gsub("NA","",Exp.Metadata_ModExp$PrimSecThird)

# read data
Data_matrix_ModExp <- Master.data_matrix %>% dplyr::select(Exp.Metadata_ModExp$Sample.Name)

dds.timeseries_MODExp <- DESeqDataSetFromMatrix(countData = Data_matrix_ModExp,
                                         colData = Exp.Metadata_ModExp, design = ~ 1) # DESeq Data Set (dds)


# (3) Exposure to severe treatment
# ncol(Data_matrix_SevExp) # 91

# metadata
Exp.Metadata_SevExp   <- Master.Exp.Metadata_2 %>%  
  dplyr::filter(!Second_Treament %in% 'M') %>% 
  dplyr::mutate(PrimSecThird = paste( (gsub("_.*","", Day_Treatment)), "_", Primary_Treatment, Second_Treament, Third_Treatment, sep=''))
Exp.Metadata_SevExp$PrimSecThird <-  gsub("NA","",Exp.Metadata_SevExp$PrimSecThird)

# read data
Data_matrix_SevExp <- Master.data_matrix %>% dplyr::select(Exp.Metadata_SevExp$Sample.Name)

dds.timeseries_SevExp <- DESeqDataSetFromMatrix(countData = Data_matrix_SevExp,
                                                colData = Exp.Metadata_SevExp, design = ~ 1) # DESeq Data Set (dds)
...
# (2) Exposure to moderate treatment
dds.MODExp_vst <- vst(dds.timeseries_MODExp) # transform it vst
dds.MODExp_vst <- assay(dds.MODExp_vst) # call only the transformed coutns in the dds object
dds.MODExp_vst <- t(dds.MODExp_vst) # transpose columns to rows and vice versa
# (3) Exposure to severe treatment
dds.SevExp_vst <- vst(dds.timeseries_SevExp) # transform it vst
dds.SevExp_vst <- assay(dds.SevExp_vst) # call only the transformed coutns in the dds object
dds.SevExp_vst <- t(dds.SevExp_vst) # transpose columns to rows and vice versa
...
par(cex = 0.6);
par(mar = c(0,4,2,0))
png("Analysis/Output/WGCNA/subseq_treatments_all/TimeSeries/All_ClusterTree_Precut.png", 1000, 1000, pointsize=20)
...
omittedsamples = (clust==0) # view the ommited samples from the tree
...
dds.timeseries_vst = dds.timeseries_vst[keepSamples, ] # integrate keepsamples to the main read dataset
...
png("Analysis/Output/WGCNA/subseq_treatments_all/TimeSeries/All_ClusterTree_Postcut.png", 1000, 1000, pointsize=20) # plot the tree with the 'keep samples' bollean (T/F) to ommit outlier samples
...
dim(dds.MODExp_vst) #  7933 genes;  91   samples
gsg = goodSamplesGenes(dds.MODExp_vst, verbose = 3);  # We first check for genes and samples with too many missing values:
gsg$allOK # If the statement returns TRUE, all genes have passed the cuts. 
sampleTree_mod = hclust(dist(dds.MODExp_vst), method = "average") # Next we cluster the samples (in contrast to clustering genes that will come later)  to see if there are any obvious outliers.

sizeGrWindow(12,9) # The user should change the dimensions if the window is too large or too small.
par(cex = 0.6);
par(mar = c(0,4,2,0))
png("Analysis/Output/WGCNA/subseq_treatments_all/TimeSeries/ModExposures_ClusterTree_Precut.png", 1000, 1000, pointsize=20)
plot(sampleTree_mod, main = "Sample clustering to detect outliers", sub="", xlab="", cex.lab = 1.5, 
     cex.axis = 1.5, cex.main = 2) + # appears there are two outliers SG55 and SG 105; can remove by hand or an automatic appraoch 
  abline(h = 100, col = "red") # add line to plot to show the cut-off od outlier samples (40000) SG105 and SG55
dev.off()

clust = cutreeStatic(sampleTree_mod, cutHeight = 85, minSize = 10) # Determine cluster under the line
table(clust) # 0 = cut; 1 = kept; says it will cut 2 and save 60
keepSamples = (clust==1) # call the 30 from clust at position 1
omittedsamples = (clust==0) # view the ommited samples from the tree
omSamples = dds.MODExp_vst[omittedsamples, ]
nrow(omSamples) # should be 16
rownames(omSamples) # should be 16 - "SG141" "SG103" "SG54"  "SG126" "SG133" "SG52"  "SG115" "SG30"  "SG51"  "SG55"  "SG53"  "SG124" "SG29"  "SG110" "SG105" "SG95"  "SG114" What treatments are these??
omSamplesData <- Exp.Metadata_ModExp %>% dplyr::filter(Sample.Name %in% (rownames(omSamples))) %>% dplyr::select(c('Sample.Name', 'Primary_Treatment', 'Day_Treatment')) # three Primary Ambient and three Primary Moderate
dds.MODExp_vst = dds.MODExp_vst[keepSamples, ] # integrate keepsamples to the main read dataset
nGenes = ncol(dds.MODExp_vst) # number of genes == 7933
nSamples = nrow(dds.MODExp_vst) # number of samples == 75  - the cut tree removed 16 samples

sampleTree_mod2 = hclust(dist(dds.MODExp_vst), method = "average") # Next we cluster the samples (in contrast to clustering genes that will come later)  to see if there are any obvious outliers.
png("Analysis/Output/WGCNA/subseq_treatments_all/TimeSeries/ModExposures_ClusterTree_Postcut.png", 1000, 1000, pointsize=20) # plot the tree with the 'keep samples' bollean (T/F) to ommit outlier samples
plot(sampleTree_mod2, main = "Sample clustering to detect outliers", sub="", xlab="", cex.lab = 1.5,
     cex.axis = 1.5, cex.main = 2)
dev.off()

TimeSeries.Samples_mod = rownames(dds.MODExp_vst);# start new variable 'd21.Samples' calling the row names of the gene data (sample as 'Mouse' in trait data)
TreatRows_mod = match(TimeSeries.Samples_mod, Exp.Metadata_ModExp$Sample.Name); # match the names
TimeSeries.Traits_mod = Exp.Metadata_ModExp[TreatRows_mod, -1]; # removes the row numbers
rownames(TimeSeries.Traits_mod) = Exp.Metadata_ModExp[TreatRows_mod, 1]; # inserts the new traitRows - matches sample treatments
dim(TimeSeries.Traits_mod) #  75  8
all(rownames(TimeSeries.Traits_mod) == rownames(dds.MODExp_vst))  # should be TRUE



# (3) Exposure to severe treatment :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::

dim(dds.SevExp_vst) #  7933 genes;  91   samples
gsg = goodSamplesGenes(dds.SevExp_vst, verbose = 3);  # We first check for genes and samples with too many missing values:
gsg$allOK # If the statement returns TRUE, all genes have passed the cuts. 
sampleTree_Sev = hclust(dist(dds.SevExp_vst), method = "average") # Next we cluster the samples (in contrast to clustering genes that will come later)  to see if there are any obvious outliers.

sizeGrWindow(12,9) # The user should change the dimensions if the window is too large or too small.
par(cex = 0.6);
par(mar = c(0,4,2,0))
png("Analysis/Output/WGCNA/subseq_treatments_all/TimeSeries/SevExposures_ClusterTree_Precut.png", 1000, 1000, pointsize=20)
plot(sampleTree_Sev, main = "Sample clustering to detect outliers", sub="", xlab="", cex.lab = 1.5, 
     cex.axis = 1.5, cex.main = 2) + # appears there are two outliers SG55 and SG 105; can remove by hand or an automatic appraoch 
  abline(h = 120, col = "red") # add line to plot to show the cut-off od outlier samples (40000) SG105 and SG55
dev.off()

clust = cutreeStatic(sampleTree_Sev, cutHeight = 120, minSize = 10) # Determine cluster under the line
table(clust) # 0 = cut; 1 = kept; says it will cut 2 and save 60
keepSamples = (clust==1) # call the 30 from clust at position 1
omittedsamples = (clust==0) # view the ommited samples from the tree
omSamples = dds.SevExp_vst[omittedsamples, ]
nrow(omSamples) # should be 12
rownames(omSamples) # should be 16 - "SG141" "SG103" "SG54"  "SG126" "SG133" "SG52"  "SG115" "SG30"  "SG51"  "SG55"  "SG53"  "SG124" "SG29"  "SG110" "SG105" "SG95"  "SG114" What treatments are these??
omSamplesData <- Exp.Metadata_SevExp %>% dplyr::filter(Sample.Name %in% (rownames(omSamples))) %>% dplyr::select(c('Sample.Name', 'Primary_Treatment', 'Day_Treatment')) # three Primary Ambient and three Primary Severate
dds.SevExp_vst = dds.SevExp_vst[keepSamples, ] # integrate keepsamples to the main read dataset
nGenes = ncol(dds.SevExp_vst) # number of genes == 7933
nSamples = nrow(dds.SevExp_vst) # number of samples == 75  - the cut tree removed 12 samples

sampleTree_Sev2 = hclust(dist(dds.SevExp_vst), method = "average") # Next we cluster the samples (in contrast to clustering genes that will come later)  to see if there are any obvious outliers.
png("Analysis/Output/WGCNA/subseq_treatments_all/TimeSeries/SevExposures_ClusterTree_Postcut.png", 1000, 1000, pointsize=20) # plot the tree with the 'keep samples' bollean (T/F) to ommit outlier samples
plot(sampleTree_Sev2, main = "Sample clustering to detect outliers", sub="", xlab="", cex.lab = 1.5,
     cex.axis = 1.5, cex.main = 2)
dev.off()

TimeSeries.Samples_Sev = rownames(dds.SevExp_vst);# start new variable 'd21.Samples' calling the row names of the gene data (sample as 'Mouse' in trait data)
TreatRows_Sev = match(TimeSeries.Samples_Sev, Exp.Metadata_SevExp$Sample.Name); # match the names
TimeSeries.Traits_Sev = Exp.Metadata_SevExp[TreatRows_Sev, -1]; # removes the row numbers
rownames(TimeSeries.Traits_Sev) = Exp.Metadata_SevExp[TreatRows_Sev, 1]; # inserts the new traitRows - matches sample treatments
dim(TimeSeries.Traits_Sev) #  75  8
all(rownames(TimeSeries.Traits_Sev) == rownames(dds.SevExp_vst))  # should be TRUE


# Save trait data - cut to from the pre/post sample tree
write.csv(TimeSeries.Traits,     file = "Analysis/Output/WGCNA/subseq_treatments_all/TimeSeries/All.TraitData_cut.csv") 
write.csv(TimeSeries.Traits_mod, file = "Analysis/Output/WGCNA/subseq_treatments_all/TimeSeries/ModExposures.TraitData_cut.csv") 
write.csv(TimeSeries.Traits_Sev, file = "Analysis/Output/WGCNA/subseq_treatments_all/TimeSeries/SevExposures.TraitData_cut.csv") 
...
# group   
Day_Treat <-  Exp.Metadata_ModExp %>% dplyr::select('Day_Treatment')
...
Day_Treat_mod <-  TimeSeries.Traits_mod %>% dplyr::select('PrimSecThird')

Day_Treat_mod$Day7_AA   <- (Day_Treat_mod$PrimSecThird == "Day7_AA")
Day_Treat_mod$Day7_AA   <- as.numeric(Day_Treat_mod$Day7_AA)
Day_Treat_mod$Day7_AM   <- (Day_Treat_mod$PrimSecThird == "Day7_AM")
Day_Treat_mod$Day7_AM   <- as.numeric(Day_Treat_mod$Day7_AM)
Day_Treat_mod$Day7_MA   <- (Day_Treat_mod$PrimSecThird == "Day7_MA")
Day_Treat_mod$Day7_MA   <- as.numeric(Day_Treat_mod$Day7_MA)
Day_Treat_mod$Day7_MM   <- (Day_Treat_mod$PrimSecThird == "Day7_MM")
Day_Treat_mod$Day7_MM   <- as.numeric(Day_Treat_mod$Day7_MM)

Day_Treat_mod$Day14_AA   <- (Day_Treat_mod$PrimSecThird == "Day14_AA")
Day_Treat_mod$Day14_AA   <- as.numeric(Day_Treat_mod$Day14_AA)
Day_Treat_mod$Day14_AM   <- (Day_Treat_mod$PrimSecThird == "Day14_AM")
Day_Treat_mod$Day14_AM   <- as.numeric(Day_Treat_mod$Day14_AM)
Day_Treat_mod$Day14_MA   <- (Day_Treat_mod$PrimSecThird == "Day14_MA")
Day_Treat_mod$Day14_MA   <- as.numeric(Day_Treat_mod$Day14_MA)
Day_Treat_mod$Day14_MM   <- (Day_Treat_mod$PrimSecThird == "Day14_MM")
Day_Treat_mod$Day14_MM   <- as.numeric(Day_Treat_mod$Day14_MM)


Day_Treat_mod$Day21_AAA   <- (Day_Treat_mod$PrimSecThird == "Day21_AAA")
Day_Treat_mod$Day21_AAA   <- as.numeric(Day_Treat_mod$Day21_AAA)
Day_Treat_mod$Day21_AAM   <- (Day_Treat_mod$PrimSecThird == "Day21_AAM")
Day_Treat_mod$Day21_AAM   <- as.numeric(Day_Treat_mod$Day21_AAM)
Day_Treat_mod$Day21_AMA   <- (Day_Treat_mod$PrimSecThird == "Day21_AMA")
Day_Treat_mod$Day21_AMA   <- as.numeric(Day_Treat_mod$Day21_AMA)
Day_Treat_mod$Day21_AMM   <- (Day_Treat_mod$PrimSecThird == "Day21_AMM")
Day_Treat_mod$Day21_AMM   <- as.numeric(Day_Treat_mod$Day21_AMM)

Day_Treat_mod$Day21_MAA   <- (Day_Treat_mod$PrimSecThird == "Day21_MAA")
Day_Treat_mod$Day21_MAA   <- as.numeric(Day_Treat_mod$Day21_MAA)
Day_Treat_mod$Day21_MAM   <- (Day_Treat_mod$PrimSecThird == "Day21_MAM")
Day_Treat_mod$Day21_MAM   <- as.numeric(Day_Treat_mod$Day21_MAM)
Day_Treat_mod$Day21_MMA   <- (Day_Treat_mod$PrimSecThird == "Day21_MMA")
Day_Treat_mod$Day21_MMA   <- as.numeric(Day_Treat_mod$Day21_MMA)
Day_Treat_mod$Day21_MMM   <- (Day_Treat_mod$PrimSecThird == "Day21_MMM")
Day_Treat_mod$Day21_MMM   <- as.numeric(Day_Treat_mod$Day21_MMM)

Day_Treat_mod           <- Day_Treat_mod[,c(2:17)] # final dataset of 0,1 for teatment groups - group groups only!


# (3) Exposure to severe treatment :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::

Day_Treat_sev <-  TimeSeries.Traits_Sev %>% dplyr::select('PrimSecThird')

Day_Treat_sev$Day7_AA   <- (Day_Treat_sev$PrimSecThird == "Day7_AA")
Day_Treat_sev$Day7_AA   <- as.numeric(Day_Treat_sev$Day7_AA)
Day_Treat_sev$Day7_AS   <- (Day_Treat_sev$PrimSecThird == "Day7_AS")
Day_Treat_sev$Day7_AS   <- as.numeric(Day_Treat_sev$Day7_AS)
Day_Treat_sev$Day7_MA   <- (Day_Treat_sev$PrimSecThird == "Day7_MA")
Day_Treat_sev$Day7_MA   <- as.numeric(Day_Treat_sev$Day7_MA)
Day_Treat_sev$Day7_MS   <- (Day_Treat_sev$PrimSecThird == "Day7_MS")
Day_Treat_sev$Day7_MS   <- as.numeric(Day_Treat_sev$Day7_MS)

Day_Treat_sev$Day14_AA   <- (Day_Treat_sev$PrimSecThird == "Day14_AA")
Day_Treat_sev$Day14_AA   <- as.numeric(Day_Treat_sev$Day14_AA)
Day_Treat_sev$Day14_AS   <- (Day_Treat_sev$PrimSecThird == "Day14_AS")
Day_Treat_sev$Day14_AS   <- as.numeric(Day_Treat_sev$Day14_AS)
Day_Treat_sev$Day14_MA   <- (Day_Treat_sev$PrimSecThird == "Day14_MA")
Day_Treat_sev$Day14_MA   <- as.numeric(Day_Treat_sev$Day14_MA)
Day_Treat_sev$Day14_MS   <- (Day_Treat_sev$PrimSecThird == "Day14_MS")
Day_Treat_sev$Day14_MS   <- as.numeric(Day_Treat_sev$Day14_MS)


Day_Treat_sev$Day21_AAA   <- (Day_Treat_sev$PrimSecThird == "Day21_AAA")
Day_Treat_sev$Day21_AAA   <- as.numeric(Day_Treat_sev$Day21_AAA)
Day_Treat_sev$Day21_AAM   <- (Day_Treat_sev$PrimSecThird == "Day21_AAM")
Day_Treat_sev$Day21_AAM   <- as.numeric(Day_Treat_sev$Day21_AAM)
Day_Treat_sev$Day21_ASA   <- (Day_Treat_sev$PrimSecThird == "Day21_ASA")
Day_Treat_sev$Day21_ASA   <- as.numeric(Day_Treat_sev$Day21_ASA)
Day_Treat_sev$Day21_ASM   <- (Day_Treat_sev$PrimSecThird == "Day21_ASM")
Day_Treat_sev$Day21_ASM   <- as.numeric(Day_Treat_sev$Day21_ASM)

Day_Treat_sev$Day21_MAA   <- (Day_Treat_sev$PrimSecThird == "Day21_MAA")
Day_Treat_sev$Day21_MAA   <- as.numeric(Day_Treat_sev$Day21_MAA)
Day_Treat_sev$Day21_MAM   <- (Day_Treat_sev$PrimSecThird == "Day21_MAM")
Day_Treat_sev$Day21_MAM   <- as.numeric(Day_Treat_sev$Day21_MAM)
Day_Treat_sev$Day21_MSA   <- (Day_Treat_sev$PrimSecThird == "Day21_MSA")
Day_Treat_sev$Day21_MSA   <- as.numeric(Day_Treat_sev$Day21_MSA)
Day_Treat_sev$Day21_MSM   <- (Day_Treat_sev$PrimSecThird == "Day21_MSM")
Day_Treat_sev$Day21_MSM   <- as.numeric(Day_Treat_sev$Day21_MSM)

Day_Treat_sev           <- Day_Treat_sev[,c(2:17)] # final dataset of 0,1 for teatment groups - group groups only!
...
# Primary treatment  :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
png("Analysis/Output/WGCNA/subseq_treatments_all/TimeSeries/All_ClusterTree_PrimaryTreatment.png", 1000, 1000, pointsize=20)
...
# Time/Days  :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
png("Analysis/Output/WGCNA/subseq_treatments_all/TimeSeries/All_ClusterTree_DaysTime.png", 1000, 1000, pointsize=20)
...
png("Analysis/Output/WGCNA/subseq_treatments_all/TimeSeries/All_ClusterTree_DayTreatment.png", 1000, 1000, pointsize=20)
...
# Group time and primary treatment (primary x time/days)  :::::::::::::::::::
png("Analysis/Output/WGCNA/subseq_treatments_all/TimeSeries/ModExposures_ClusterTree_DayTreatment.png", 1000, 1000, pointsize=20)
traitColors_Day_Treat_MOD = labels2colors(Day_Treat_mod); # Convert traits to a color representation: white means low, red means high, grey means missing entry
plotDendroAndColors(sampleTree_mod2, traitColors_Day_Treat_MOD, # Plot the sample dendrogram and the colors underneath.
                    groupLabels = names(traitColors_Day_Treat_MOD), 
                    main = "Sample dendrogram and trait heatmap")
dev.off()

# Group time and primary treatment (primary x time/days)  :::::::::::::::::::
png("Analysis/Output/WGCNA/subseq_treatments_all/TimeSeries/SevExposures_ClusterTree_DayTreatment.png", 1000, 1000, pointsize=20)
traitColors_Day_Treat_SEV = labels2colors(Day_Treat_sev); # Convert traits to a color representation: white means low, red means high, grey means missing entry
plotDendroAndColors(sampleTree_mod2, traitColors_Day_Treat_SEV, # Plot the sample dendrogram and the colors underneath.
                    groupLabels = names(traitColors_Day_Treat_SEV), 
                    main = "Sample dendrogram and trait heatmap")
dev.off()


# write the vst transformed data 
write.csv(dds.timeseries_vst, "Analysis/Output/WGCNA/subseq_treatments_all/TimeSeries/All_vstTransformed_WGCNAdata.csv") # write
write.csv(dds.MODExp_vst, "Analysis/Output/WGCNA/subseq_treatments_all/TimeSeries/ModExposures_vstTransformed_WGCNAdata.csv") # write
write.csv(dds.SevExp_vst, "Analysis/Output/WGCNA/subseq_treatments_all/TimeSeries/SevExposures_vstTransformed_WGCNAdata.csv") # write
...
sft = pickSoftThreshold(dds.timeseries_vst, powerVector = powers, verbose = 5) # Call the network topology analysis function ...wait for this to finish

sizeGrWindow(9, 5) # set window size 
png("Analysis/Output/WGCNA/subseq_treatments_all/TimeSeries/All_ScaleTopology_SoftThresh.png", 1000, 1000, pointsize=20) # png to output 
par(mfrow = c(1,2));
cex1 = 0.9;
plot(sft$fitIndices[,1], -sign(sft$fitIndices[,3])*sft$fitIndices[,2], # Scale-free topology fit index as a function of the soft-thresholding power
    ...
    text(sft$fitIndices[,1], -sign(sft$fitIndices[,3])*sft$fitIndices[,2],
        ...
        abline(h=0.90,col="red") # # this line corresponds to using an R^2 cut-off of h - look at at cut off at power of 6
...
dim(dds.MODExp_vst) #75 7933

sft = pickSoftThreshold(dds.MODExp_vst, powerVector = powers, verbose = 5) # Call the network topology analysis function ...wait for this to finish

sizeGrWindow(9, 5) # set window size 
png("Analysis/Output/WGCNA/subseq_treatments_all/TimeSeries/ModExposures_ScaleTopology_SoftThresh.png", 1000, 1000, pointsize=20) # png to output 
par(mfrow = c(1,2));
cex1 = 0.9;
plot(sft$fitIndices[,1], -sign(sft$fitIndices[,3])*sft$fitIndices[,2], # Scale-free topology fit index as a function of the soft-thresholding power
     xlab="Soft Threshold (power)",ylab="Scale Free Topology Model Fit,signed R^2",type="n",
...
plot(sft$fitIndices[,1], sft$fitIndices[,5], # Mean connectivity as a function of the soft-thresholding power
     ...
     main = paste("Mean connectivity"))
text(sft$fitIndices[,1], sft$fitIndices[,5], labels=powers, cex=cex1,col="red")
dev.off() # output soft threshold of 7


# (3) Exposure to severe treatment :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::

dim(dds.SevExp_vst) #  75 7933

sft = pickSoftThreshold(dds.SevExp_vst, powerVector = powers, verbose = 5) # Call the network topology analysis function ...wait for this to finish

sizeGrWindow(9, 5) # set window size 
png("Analysis/Output/WGCNA/subseq_treatments_all/TimeSeries/SevExposures_ScaleTopology_SoftThresh.png", 1000, 1000, pointsize=20) # png to output 
par(mfrow = c(1,2));
cex1 = 0.9;
plot(sft$fitIndices[,1], -sign(sft$fitIndices[,3])*sft$fitIndices[,2], # Scale-free topology fit index as a function of the soft-thresholding power
     xlab="Soft Threshold (power)",ylab="Scale Free Topology Model Fit,signed R^2",type="n",
...
plot(sft$fitIndices[,1], sft$fitIndices[,5], # Mean connectivity as a function of the soft-thresholding power
     ...
     main = paste("Mean connectivity"))
text(sft$fitIndices[,1], sft$fitIndices[,5], labels=powers, cex=cex1,col="red")
dev.off() # output soft threshold of 8
...
adjacency_sign = adjacency(dds.timeseries_vst, power = softPower, type="signed") # signed; this takes a long time.. just wait...

# (2) Exposure to moderate treatment :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
softPower_mod = 7 # set your soft threshold based on the plots above 
adjacency_sign_ModExp = adjacency(dds.MODExp_vst, power = softPower_mod, type="signed") # signed; this takes a long time.. just wait...


# (3) Exposure to severe treatment :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
softPower_sev = 8 # set your soft threshold based on the plots above 
adjacency_sign_SevExp = adjacency(dds.SevExp_vst, power = softPower_sev, type="signed") # signed; this takes a long time.. just wait...
...
TOM_sign_ModExp       = TOMsimilarity(adjacency_sign_ModExp, TOMType="signed")  # this takes a long time.. just wait...
dissTOM_sign_ModExp   = 1-TOM_sign_ModExp

# (3) Exposure to severe treatment :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::

TOM_sign_SevExp       = TOMsimilarity(adjacency_sign_SevExp, TOMType="signed")  # this takes a long time.. just wait...
dissTOM_sign_SevExp   = 1-TOM_sign_SevExp
...
plot(geneTree_sign, xlab="", sub="", main = "Gene clustering on TOM-based dissimilarity", # Plot the resulting clustering tree (dendrogram)
     labels = FALSE, hang = 0.04);

# (2) Exposure to moderate treatment :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
geneTree_sign_ModExp   = hclust(as.dist(dissTOM_sign_ModExp), method = "average");
plot(geneTree_sign_ModExp, xlab="", sub="", main = "Gene clustering on TOM-based dissimilarity", # Plot the resulting clustering tree (dendrogram)
     labels = FALSE, hang = 0.04);

# (3) Exposure to severe treatment :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
geneTree_sign_SevExp   = hclust(as.dist(dissTOM_sign_SevExp), method = "average");
plot(geneTree_sign_SevExp, xlab="", sub="", main = "Gene clustering on TOM-based dissimilarity", # Plot the resulting clustering tree (dendrogram)
     labels = FALSE, hang = 0.04);
#=====================================================================================
#
...
# (2) Exposure to moderate treatment :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
dynamicMods_sign_ModExp = cutreeDynamic(dendro = geneTree_sign_ModExp, distM = dissTOM_sign_ModExp,
                                 deepSplit = 1, pamRespectsDendro = FALSE,
                                 minClusterSize = minModuleSize);
table(dynamicMods_sign_ModExp) # number of genes per module - signed modules - more of them here all over 300 genes 

# (3) Exposure to severe treatment :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
dynamicMods_sign_SevExp = cutreeDynamic(dendro = geneTree_sign_SevExp, distM = dissTOM_sign_SevExp,
                                 deepSplit = 1, pamRespectsDendro = FALSE,
                                 minClusterSize = minModuleSize);
table(dynamicMods_sign_SevExp) # number of genes per module - signed modules - more of them here all over 300 genes 
#=====================================================================================
#
...
# (1) Full data all samples :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
dynamicColors_sign = labels2colors(dynamicMods_sign) #Convert numeric lables into colors - add colors to module labels (previously numbers)
table(dynamicColors_sign) # lets look at this table...
png("Analysis/Output/WGCNA/subseq_treatments_all/TimeSeriesAll_ClusterDendrogram_signedTOM.png", 1000, 1000, pointsize=20)
...
# (2) Exposure to moderate treatment :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
dynamicColors_sign_ModExp = labels2colors(dynamicMods_sign_ModExp) #Convert numeric lables into colors - add colors to module labels (previously numbers)
table(dynamicMods_sign_ModExp) # lets look at this table...
png("Analysis/Output/WGCNA/subseq_treatments_all/TimeSeries/ModExposures_ClusterDendrogram_signedTOM.png", 1000, 1000, pointsize=20)
plotDendroAndColors(geneTree_sign_ModExp, dynamicColors_sign_ModExp, "Dynamic Tree Cut", 
                    dendroLabels = FALSE, hang = 0.03,
                    addGuide = TRUE, guideHang = 0.05,
                    main = "Gene dendrogram and module colors 'SIGNED'") # signed TOM - now segmented well with minimal noise in major modules 
dev.off() # output the signed TOM dendrogram for comparison with the signed TOM (below and after merging - if needed!) 
...
# (3) Exposure to severe treatment :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
dynamicColors_sign_SevExp = labels2colors(dynamicMods_sign_SevExp) #Convert numeric lables into colors - add colors to module labels (previously numbers)
table(dynamicMods_sign_SevExp) # lets look at this table...
png("Analysis/Output/WGCNA/subseq_treatments_all/TimeSeries/SevExposures_ClusterDendrogram_signedTOM.png", 1000, 1000, pointsize=20)
plotDendroAndColors(geneTree_sign_SevExp, dynamicColors_sign_SevExp, "Dynamic Tree Cut", 
                    dendroLabels = FALSE, hang = 0.03,
                    addGuide = TRUE, guideHang = 0.05,
                    main = "Gene dendrogram and module colors 'SIGNED'") # signed TOM - now segmented well with minimal noise in major modules 
dev.off() # output the signed TOM dendrogram for comparison with the signed TOM (below and after merging - if needed!) 
...
#
#=====================================================================================
# Call an automatic merging function - IF NEEDED! - ONLY  if you had a cut height in the previous cluster...
...
pdf("Analysis/Output/WGCNA/subseq_treatments_all/TimeSeries/heatmaps/All_Treatments_Primary_heatmap.pdf", width=5, height=6) # this heatmap looks better
...
# (3) Exposure to moderate treatment :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
