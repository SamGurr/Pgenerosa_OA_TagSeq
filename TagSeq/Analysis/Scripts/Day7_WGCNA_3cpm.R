...
day7.counts.matrix <- read.csv(file="Analysis/Data/Filtered_Counts/3cpm_50perc/day7.counts.filtered_3cpm50perc.csv", sep=',', header=TRUE)
...
png("Analysis/Output/WGCNA/3cpm/Day7/Day7_ClusterTree_Precut.png", 1000, 1000, pointsize=20)
...
rownames(omSamples) # should be 6 - "SG21" "SG22" "SG59" What treatments are these??
...
nGenes = ncol(dds.d7_vst) # number of genes == 14082 
nSamples = nrow(dds.d7_vst) # number of samples == 33  - cut tree removed 3 samples 
...
png("Analysis/Output/WGCNA/3cpm/Day7/Day7_ClusterTree_Postcut.png", 1000, 1000, pointsize=20)
...
dim(dds.d7_vst) #   33 14082- transformed count data now  has 30 samples - 6 cut in the tree step above 
...
dim(d7.Traits) #  33 Samples 8 columns (primary and second treatment) - note: no Third treatment because that occured Days 14-21, after this sampling day
...
# All Traits
png("Analysis/Output/WGCNA/3cpm/Day7/Day7_ClusterTree_AllTraits.png", 1000, 1000, pointsize=20)
...
png("Analysis/Output/WGCNA/3cpm/Day7/Day7_ClusterTree_PrimaryTreatment.png", 1000, 1000, pointsize=20)
...
# Second treatment 
png("Analysis/Output/WGCNA/3cpm/Day7/Day7_ClusterTree_SecondTreatment.png", 1000, 1000, pointsize=20)
...
png("Analysis/Output/WGCNA/3cpm/Day7/Day7_ClusterTree_GroupTreatment.png", 1000, 1000, pointsize=20)
...
# Phys Only
png("Analysis/Output/WGCNA/3cpm/Day7/Day7_ClusterTree_PhysOnly.png", 1000, 1000, pointsize=20)
...
# save data
save(dds.d7_vst, d7.Traits, file = "Analysis/Output/WGCNA/3cpm/Day7/d.7-dataInput.RData")
save(dds.d7_vst, d7.Traits.phys, file = "Analysis/Output/WGCNA/3cpm/Day7/d.7-dataInput_treatmentonly.RData")
save(dds.d7_vst, d7.Traits.treat, file = "Analysis/Output/WGCNA/3cpm/Day7/d.7-dataInput_physonly.RData")


# write the vst transformed data 
write.csv(dds.d7_vst, "Analysis/Output/WGCNA/3cpm/Day7/Day7_vstTransformed_WGCNAdata.csv") # write
...
# png to output 
png("Analysis/Output/WGCNA/3cpm/Day7/Day7_ScaleTopology_SoftThresh.png", 1000, 1000, pointsize=20)
par(mfrow = c(1,2));
cex1 = 0.9;
...
# https://www.rdocumentation.org/packages/WGCNA/3cpm/10cpm/versions/1.69/topics/adjacency
# https://ramellose.github.io/networktutorials/wgcna.html
# https://horvath.genetics.ucla.edu/html/CoexpressionNetwork/Rpackages/WGCNA/3cpm/10cpm/TechnicalReports/signedTOM.pdf
#=====================================================================================
...
# Plot the result
sizeGrWindow(7, 6)
png("Analysis/Output/WGCNA/3cpm/Day7/Day7_ClusterEigengenes.png", 1000, 1000, pointsize=20)
...
png("Analysis/Output/WGCNA/3cpm/Day7/Day7_ClusterDendrogram_signed.png", 1000, 1000, pointsize=20)
...
save(MEs, moduleLabels, moduleColors, geneTree, file = "Analysis/Output/WGCNA/3cpm/Day7/Day7-networkConstruction-stepByStep.RData")
# write csv - save the module eigengenes
write.csv(MEs, file = "Analysis/Output/WGCNA/3cpm/Day7/d7.WGCNA_ModulEigengenes.csv")
table(mergedColors)
...
#                        TOMType = "signed",  
#                        saveTOMs = TRUE,
#                        saveTOMFileBase = "Analysis/Output/WGCNA/3cpm/10cpm/Day7/d7.Geoduck", 
...
# 
# # write csv - save the module eigengenes
# write.csv(MEs, file = "Analysis/Output/WGCNA/3cpm/10cpm/Day7/d7.WGCNA_ModulEigengenes.csv")
# 
...
png("Analysis/Output/WGCNA/3cpm/Day7/heatmaps/Day7_Treatments_Primary_heatmap2.png", 500, 1000, pointsize=20)
...
png("Analysis/Output/WGCNA/3cpm/Day7/Day7_Treatments_Primary_heatmap.png", 500, 1000, pointsize=20)
pdf("Analysis/Output/WGCNA/3cpm/Day7/heatmaps/Day7_Treatments_Primary_heatmap.pdf", width=5, height=6)
...
par(mar = c(8, 9.5, 5, 3));
# Display the correlation values within a heatmap plot
png("Analysis/Output/WGCNA/3cpm/Day7/heatmaps/Day7_Treatments_Second_heatmap2.png", 1000, 500, pointsize=20)
...
png("Analysis/Output/WGCNA/3cpm/Day7/heatmaps/Day7_Treatments_Second_heatmap.png", 500, 1000, pointsize=20)
pdf("Analysis/Output/WGCNA/3cpm/Day7/heatmaps/Day7_Treatments_Second_heatmap.pdf", width=5, height=6)
...
par(mar = c(8, 9.5, 5, 3));
# Display the correlation values within a heatmap plot
png("Analysis/Output/WGCNA/3cpm/Day7/heatmaps/Day7_Group_heatmap2.png", 1000, 500, pointsize=20)
...
png("Analysis/Output/WGCNA/3cpm/Day7/heatmaps/Day7_Group_heatmap.png", 500, 1000, pointsize=20)
pdf("Analysis/Output/WGCNA/3cpm/Day7/heatmaps/Day7_Group_heatmap.pdf", width=5, height=6)
...
#plot it
png("Analysis/Output/WGCNA/3cpm/Day7/Day7_ME_Boxplot.png", 600, 1000, pointsize=20)
...
length(colnames(dds.d7_vst)[moduleColors=="brown"]) # 2349 total genes in the violet module
length(colnames(dds.d7_vst)[moduleColors=="blue"]) # 4317 total genes in the violet module
...
yellow_mod <- geneInfo_GROUPS %>%  dplyr::filter(moduleColor %in% 'yellow')
# red <- red_mod[,c(3,4:7)]
...
length(colnames(dds.d7_vst)[moduleColors=="blue"]) # 595 total genes in the black module
black_mod <- geneInfo_GROUPS %>%  dplyr::filter(moduleColor %in% 'blue')
nrow(black_mod) # 595 total genes in the black module
black_mod_05 <- black_mod #%>%  dplyr::filter(p.MM.black < 0.05) 
...
write.csv(geneInfo_GROUPS, file = "Analysis/Output/WGCNA/3cpm/Day7/d7.WGCNA_ModulMembership.csv")
...
# Load data 
d7_ModEigen <- read.csv("Analysis/Output/WGCNA/3cpm/Day7/d7.WGCNA_ModulEigengenes.csv")
d7_Annot_ModuleMembership <-  read.csv("Analysis/Output/WGCNA/3cpm/Day7/d7.WGCNA_ModulMembership.csv")
d7_vst_data <- read.csv("Analysis/Output/WGCNA/3cpm/Day7/Day7_vstTransformed_WGCNAdata.csv")
...
dim(d7_Annot_ModuleMembership) #   14082    26
...
phys <- d7_EigenTraitMaster[,c(13:18)]
modules <- d7_EigenTraitMaster[,c(2:5)]
...
for(i in 1:ncol(modules)) {
  ...
  # save plot grid 
  #png("Analysis/Output/WGCNA/3cpm/10cpm/Day21/Day21_PhysOnly_heatmap2.png", 1000, 1000, pointsize=20)
  ggarrange(p1, p2, p3, p4, p5, p6, ncol = 2, nrow = 3)
  ggsave(paste0("Analysis/Output/WGCNA/3cpm/Day7/EigengenePlots/Day7_EigengenePlot_", module_color, ".png"))
}
...
for(i in 1:nrow(modcolor)) {
  ...
  pdf(paste("Analysis/Output/WGCNA/3cpm/Day7/ModuleExpression_Treatment/day7_Exp_Module",modcolor[i,],".pdf"), width=6, height=6)
...
for(i in 1:nrow(modcolor)) {
  ...
  dir.create(paste0("Analysis/Output/WGCNA/3cpm/Day7/ModuleExpresion_Phenotype/",module_color), showWarnings = FALSE)
  ...
  png(paste("Analysis/Output/WGCNA/3cpm/Day7/ModuleExpresion_Phenotype/",module_color,"/Day7_Exp_Module_",module_color, "_",(names(phys)[m]),".png", sep = ''), 600, 1000, pointsize=20)
...
for(i in 1:nrow(modcolor)) {
  ...
  pdf(paste("Analysis/Output/WGCNA/3cpm/10cpm/Day7/goseq_modules/Day7_goseq_tiles_ME",modcolor[i,],".pdf", sep =''), width=25, height=50)
  ...
  pdf(paste("Analysis/Output/WGCNA/3cpm/10cpm/Day7/goseq_modules/Day7_goseq_ME",modcolor[i,],".pdf", sep =''), width=8, height=5)
  print(Plot)
  dev.off()
}
...
# 
# 
# MFplot_red <- red_MF %>% mutate(term = fct_reorder(term, (-log10(over_represented_pvalue)) )) %>%
#   ggplot( aes(x=term, y=(-log10(over_represented_pvalue)) ) ) +
...
# 
# #Cellular Processes
# CCplot_red <- red_CC %>% mutate(term = fct_reorder(term, (-log10(over_represented_pvalue)) )) %>%
#   ggplot( aes(x=term, y=(-log10(over_represented_pvalue)) ) ) +
#   geom_segment( aes(x=term ,xend=term, y=0, yend=(-log10(over_represented_pvalue)) ), color="grey") +
#   geom_point(size=3, color="#69b3a2") +
#   coord_flip() +
...
# BPplot_red <- red_BP %>% mutate(term = fct_reorder(term,  (-log10(over_represented_pvalue)) )) %>%
#   ggplot( aes(x=term, y= (-log10(over_represented_pvalue)) ) ) +
#   geom_segment( aes(x=term ,xend=term, y=0, yend= (-log10(over_represented_pvalue)) ), color="grey") +
#   geom_point(size=3, color="#69b3a2") +
#   coord_flip() +
...
# 
# library(tidyr)
# pdf("Analysis/Output/WGCNA/3cpm/10cpm/Day7/goseq_modules/Day7_goseq_MEred.pdf", width=8, height=5)
...
#   geom_label(aes(x = 2, y = 4, label = paste(num_red, "MEred genes"))) +
#   theme_bw() + #Set background color 
#   theme(panel.border = element_blank(), # Set border
#         panel.grid.major = element_blank(), #Set major gridlines
#         panel.grid.minor = element_blank(), #Set minor gridlines
