...
day14.counts.matrix <- read.csv(file="Analysis/Data/Filtered_Counts/3cpm_50perc/day14.counts.filtered_3cpm50perc.csv", sep=',', header=TRUE)
...
png("Analysis/Output/WGCNA/3cpm/Day14/Day14_ClusterTree_Precut.png", 1000, 1000, pointsize=20)
...
abline(h = 140, col = "red") # add line to plot to show the cut-off od outlier samples (40000) SG105 and SG55
dev.off()

#cut ethe tree and ommit from the reads data
clust = cutreeStatic(sampleTree, cutHeight = 140, minSize = 10) # Determine cluster under the line
...
nGenes = ncol(dds.d14_vst) # number of genes == 14406
nSamples = nrow(dds.d14_vst) # number of samples == 27  - the cut tree removed 6 samples
...
png("Analysis/Output/WGCNA/3cpm/Day14/Day14_ClusterTree_Postcut.png", 1000, 1000, pointsize=20)
...
# All Traits
png("Analysis/Output/WGCNA/3cpm/Day14/Day14_ClusterTree_AllTreatments.png", 1000, 1000, pointsize=20)
...
png("Analysis/Output/WGCNA/3cpm/Day14/Day14_ClusterTree_PrimaryTreatment.png", 1000, 1000, pointsize=20)
...
png("Analysis/Output/WGCNA/3cpm/Day14/Day14_ClusterTree_SecondTreatment.png", 1000, 1000, pointsize=20)
...
png("Analysis/Output/WGCNA/3cpm/Day14/Day14_ClusterTree_GroupTreatment.png", 1000, 1000, pointsize=20)
...
dev.off()
# save data
# save(dds.d14_vst, d14.Traits, file = "Analysis/Output/WGCNA/3cpm/Day14/d.7-dataInput.RData")
# save(dds.d14_vst, d14.Traits.phys, file = "Analysis/Output/WGCNA/3cpm/Day14/d.7-dataInput_treatmentonly.RData")
# save(dds.d14_vst, d14.Traits.treat, file = "Analysis/Output/WGCNA/3cpm/Day14/d.7-dataInput_physonly.RData")


# write the vst transformed data 
write.csv(dds.d14_vst, "Analysis/Output/WGCNA/3cpm/Day14/Day14_vstTransformed_WGCNAdata.csv") # write
...
# png to output 
png("Analysis/Output/WGCNA/3cpm/Day14/Day14_ScaleTopology_SoftThresh.png", 1000, 1000, pointsize=20)
par(mfrow = c(1,2));
cex1 = 0.9;
...
# https://www.rdocumentation.org/packages/WGCNA/3cpm/versions/1.69/topics/adjacency
# https://ramellose.github.io/networktutorials/wgcna.html
# https://horvath.genetics.ucla.edu/html/CoexpressionNetwork/Rpackages/WGCNA/3cpm/TechnicalReports/signedTOM.pdf
#=====================================================================================
...
# 3960 2302 1971 1346 1111 1038  748  558  440  435  345  152 
#=====================================================================================
#
...
# 748        2302        1971        1111         345         440         558         435        1038         152        3960        1346
...
png("Analysis/Output/WGCNA/3cpm/Day14/Day14_ClusterEigengenes_signedTOM.png", 1000, 1000, pointsize=20) # signed TOM 
plot(METree, main = "Clustering of module eigengenes (dissimilarity calc = MEDiss = 1-cor(MEs))",
     xlab = "", sub = "") +
abline(h=MEDissThres, col = "red") # add the line to the tree
dev.off()
...
merge = mergeCloseModules(dds.d14_vst, dynamicColors_sign, cutHeight = 0.55, verbose = 3) # signed TOM 
...
#=====================================================================================
sizeGrWindow(12, 9)
png("Analysis/Output/WGCNA/3cpm/Day14/Day14_ClusterDendrogram_signedTOM.png", 1000, 1000, pointsize=20) # signed TOM 
...
table(moduleColors) 
# blue     brown     green   magenta    purple       red turquoise    yellow 
# 2647      1971      1669       440      1335      1038      3960      1346 
...
# save(MEs, moduleLabels, moduleColors, geneTree, file = "Analysis/Output/WGCNA/3cpm/Day14/Day14-networkConstruction-stepByStep.RData")
# write csv - save the module eigengenes
write.csv(MEs, file = "Analysis/Output/WGCNA/3cpm/Day14/d14.WGCNA_ModulEigengenes.csv") 
...
#                        TOMType = "signed",  
#                        saveTOMs = TRUE,
#                        saveTOMFileBase = "Analysis/Output/WGCNA/3cpm/day14/d14.Geoduck", 
...
# png("Analysis/Output/WGCNA/3cpm/day14/day14_ClusterDendrogram.png", 1000, 1000, pointsize=20)
...
nGenes = ncol(dds.d14_vst); # 14406
nSamples = nrow(dds.d14_vst); #  27
...
#plot it
png("Analysis/Output/WGCNA/3cpm/Day14/Day14_ME_Boxplot.png", 600, 1000, pointsize=20)
...
png("Analysis/Output/WGCNA/3cpm/Day14/heatmaps/Day14_Treatments_Primary_heatmap2.png", 1000, 1000, pointsize=20)
...
#png("Analysis/Output/WGCNA/3cpm/Day14/Day14_Treatments_Primary_heatmap.png", 500, 1000, pointsize=20)
pdf("Analysis/Output/WGCNA/3cpm/Day14/heatmaps/Day14_Treatments_Primary_heatmap.pdf", width=5, height=6)
...
par(mar = c(8, 9.5, 5, 3));
# Display the correlation values within a heatmap plot
png("Analysis/Output/WGCNA/3cpm/Day14/heatmaps/Day14_Treatments_Second_heatmap2.png", 500, 1000, pointsize=20)
...
#png("Analysis/Output/WGCNA/3cpm/Day14/heatmaps/Day14_Treatments_Second_heatmap.png", 500, 1000, pointsize=20)
pdf("Analysis/Output/WGCNA/3cpm/Day14/heatmaps/Day14_Treatments_Second_heatmap.pdf", width=5, height=6)
...
par(mar = c(8, 9.5, 5, 3));
# Display the correlation values within a heatmap plot
png("Analysis/Output/WGCNA/3cpm/Day14/heatmaps/Day14_Group_heatmap2.png", 1000, 1000, pointsize=20)
...
#png("Analysis/Output/WGCNA/3cpm/Day14/heatmaps/Day14_Group_heatmap.png", 500, 1000, pointsize=20)
pdf("Analysis/Output/WGCNA/3cpm/Day14/heatmaps/Day14_Group_heatmap.pdf", width=5, height=6)
...
brown_mod_05 <- brown_mod #%>%  dplyr::filter(p.MM.brown < 0.05) 
...
# magenta module 
length(colnames(dds.d14_vst)[moduleColors=="magenta"]) # 595 total genes in the magenta module
magenta_mod <- geneInfo_GROUPS %>%  dplyr::filter(moduleColor %in% 'magenta')
nrow(magenta_mod) # 595 total genes in the magenta module
magenta_mod_05 <- magenta_mod # %>%  dplyr::filter(p.MM.magenta < 0.05) 
magenta_mod_05_MM <-magenta_mod_05[,c(8:13)]
nrow(magenta_mod_05_MM) # 99 total genes in the magenta module

magenta_mod_05_MM$Gene.ID <- rownames(magenta_mod_05_MM)
magenta_mod_05_MM_melt <- melt(magenta_mod_05_MM, id=c("Gene.ID"))
magenta_mod_05_MM_melt$Primary <- substr(magenta_mod_05_MM_melt$variable, 4,4)
magenta_mod_05_MM_melt$Second <- substr(magenta_mod_05_MM_melt$variable, 5,5)
...
# black module 
length(colnames(dds.d14_vst)[moduleColors=="black"]) # 595 total genes in the black module
...
nrow(black_mod_05_MM) # 99 total genes in the black module
...
write.csv(geneInfo_GROUPS, file = "Analysis/Output/WGCNA/3cpm/Day14/d14.WGCNA_ModulMembership.csv")
...
library(ggpmisc)
# Load data 
d14_ModEigen <- read.csv("Analysis/Output/WGCNA/3cpm/Day14/d14.WGCNA_ModulEigengenes.csv")
d14_Annot_ModuleMembership <-  read.csv("Analysis/Output/WGCNA/3cpm/Day14/d14.WGCNA_ModulMembership.csv")
d14_vst_data <- read.csv("Analysis/Output/WGCNA/3cpm/Day14/day14_vstTransformed_WGCNAdata.csv")
...
phys <- d14_EigenTraitMaster[,c(21:23)]
modules <- d14_EigenTraitMaster[,c(2:10)]
...
for(i in 1:ncol(modules)) {
  ...
  # save plot grid 
  #png("Analysis/Output/WGCNA/3cpm/Day21/Day21_PhysOnly_heatmap2.png", 1000, 1000, pointsize=20)
  ggarrange(p1, p2, p3, 
            #p4, p5, p6, 
            ncol = 1, nrow = 3)
  ggsave(paste0("Analysis/Output/WGCNA/3cpm/Day14/EigengenePlots/day14_EigengenePlot_", module_color, ".png"))
}
...
for(i in 1:nrow(modcolor)) {
  ...
  pdf(paste("Analysis/Output/WGCNA/3cpm/Day14/ModuleExpression_Treatment/day14_Exp_Module",modcolor[i,],".pdf"), width=6, height=6)
...
for(i in 1:nrow(modcolor)) {
  ...
  dir.create(paste0("Analysis/Output/WGCNA/3cpm/day14/ModuleExpresion_Phenotype/",module_color), showWarnings = FALSE)
  ...
  for(m in 1:ncol(phys)) {
    ...
    png(paste("Analysis/Output/WGCNA/3cpm/day14/ModuleExpresion_Phenotype/",module_color,"/day14_Exp_Module_",module_color, "_",(names(phys)[m]),".png", sep = ''), 600, 1000, pointsize=20)
...
for(i in 1:nrow(modcolor)) {
  ...
  pdf(paste("Analysis/Output/WGCNA/3cpm/Day14/goseq_modules/Day14_goseq_tiles_ME",modcolor[i,],".pdf", sep =''), width=8, height=10)
  ...
  pdf(paste("Analysis/Output/WGCNA/3cpm/Day14/goseq_modules/Day14_goseq_ME",modcolor[i,],".pdf", sep =''), width=8, height=5)
  print(Plot)
  dev.off()
}
...
library(tidyr)
pdf("Analysis/Output/WGCNA/3cpm/Day14/goseq_modules/Day14_goseq_MEblackpink_cluster.pdf", width=12, height=5)
