...
Objective with the final list 'Const.DEGs.list' to label callouts in volcano plots 
...
d0.exp_data   <- read.csv(file="../Data/filtered_counts/day0.exp.data.csv", sep=',', header=TRUE) %>%   mutate_if(is.character, as.1factor)
...
table(res.d7.primary$padj<0.05) #  108 DEGs (same as above)
...
# Write csv
resdata.d14.primary  <- merge(as.data.frame(res.d14_primary.ordered), as.data.frame(counts(dds.d14.group, normalized=TRUE)), by="row.names", sort=FALSE) ## Merge with normalized count data
